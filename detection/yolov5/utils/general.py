...
#                 cv2.imwrite('test%i.jpg' % j, cutout)
            ...
            nm = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
            ims = nm(ims)
            
           
            pred_cls2 = model(torch.Tensor(ims).to(d.device))  # classifier prediction
            
            asd = list()
            print()
#             print(pred_cls2)
            for outs in pred_cls2:
                print(outs)
                top1_id = outs.argmax()
                print(top1_id)
                
#                 top1_conf = 
#                 confidence = torch.nn.functional.softmax(outs, dim=1)[0] * 100
