...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            # 일단은 저장된 crop images를 PIL data로 불러와 진행
            # d (ret) 을 이용하면 image 저장과정이 필요없을 것 같은데..
            # TODO 사항으로.
...
#             scale_coords(img.shape[2:], d[:, :4], im0[i].shape)
            ...
            # Classes
            data_path = 'forServiceTest/output'  # class 별 폴더로 나누어진걸 확 가져와서 라벨도 달아준다
...
#                 cv2.imwrite('forServiceTest/output/classifytest_%g.jpg' % j, cutout)
